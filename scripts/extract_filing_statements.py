...
def infer_source_arithmetic_links(rows: list[dict[str, Any]]) -> None:
    ...
    for parent_index, parent_row in enumerate(rows):
        if (
            ...
            continue
        # US cash-flow faces print the activity TOTAL one level deeper than
        # its members ("Cash generated by operating activities" indented past
        # the block rows). Same-level membership is the primary evidence and
        # is tried alone first; only when it proves nothing is the
        # one-level-shallower reading tried, as a SEPARATE pass — mixing the
        # two levels into one candidate pool mints families that cross
        # indentation boundaries.
        parent_level = int(parent_row.get("hierarchy_level") or 0)
        unique: list[list[int]] = []
        for member_level in (parent_level, parent_level - 1):
            candidates = [
                index
                for index in range(max(0, parent_index - 64), parent_index)
                if (
                    _typed_partial_series(rows[index]) is not None
                    and int(rows[index].get("hierarchy_level") or 0) == member_level
                )
            ]
            # An INVERTED total (printed deeper than its members) binds only to
            # the block it directly terminates: no typed row may sit between
            # the block and the total, or the shallower reading would let a
            # nested subtotal steal members from their own printed parent.
            if member_level < parent_level and any(
                _typed_partial_series(rows[index]) is not None
                and index not in candidates
                for index in range(
                    (candidates[-1] + 1) if candidates else parent_index,
                    parent_index,
                )
            ):
                continue
