...
const CORE_ROLE_ALIASES = Object.freeze({
  income_statement: Object.freeze({
    ...
    tax_expense: [
      "income tax expense", "tax expense", "taxation", "income taxes",
  ...
  if (filings?.entity_name) {
    ...
    identity = {
      ...
      ...(filings.accounting_framework
        ...
        : {}),
      // The fiscal calendar follows the filings, never the declaration: a
      // 52/53-week filer's year end MOVES, and only the sealed evidence knows
      // which calendar the issuer keeps. fixed_date is the schema default, so
      // it is never written — existing cases stay byte-identical.
      ...(filings.fiscal_calendar_kind && filings.fiscal_calendar_kind !== "fixed_date"
        ? { fiscal_calendar: filings.fiscal_calendar_kind }
        : {}),
    };
  }
  return {
