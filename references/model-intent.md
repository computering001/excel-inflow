...
Use exactly ONE underlying numeric binary control over the adjustment columns.
Accept only `0` or `1`; display the value as `Off/On` through number formatting
or conditional formatting. Label it for what it governs on the face — the
adjustment columns — not for the transaction that happens to populate them.

- `1 / On`: the adjustment columns carry the acquisition overlay and each
  pro-forma column equals standalone plus adjustment.
- `0 / Off`: every cell in the adjustment columns returns a formula-driven
  exact zero and every pro-forma column returns its standalone column unchanged.

Do not add a second switch for pro-forma visibility. The acquisition IS what
populates the adjustments, so an acquisition-specific control only gives the
reader two places to look for one answer. Formula text must be present and
identical in both states: the control changes the answer, never the workings.
...
Do not add target net debt, target cash, a separate purchase consideration, a
sources-and-uses schedule, a funding percentage, target-ratio controls, timing
overrides, acquisition amortisation or an acquisition maturity to the canonical
overlay. A materially different financing structure is a bespoke extension and
must be explicitly requested rather than inferred.

Do not represent the transaction as an `acquisition_debt` class in the ordinary
instrument register. Use only the single adjustment-columns control together
with the adjustment and pro-forma rows.

```text
target EBITDA = transaction value / entry EV-to-EBITDA
```

Infer full-year target revenue, D&A, EBIT, working capital, capex, tax, net
income and cash flow at 100% from the corresponding standalone forecast ratios.
Do not expose or accept a second target-ratio assumption set in the canonical
case.

For aggregate target working capital:

```text
target working-capital cash impact
= target revenue
× standalone working-capital cash impact / standalone revenue
```

Protect every inferred ratio against a zero or unavailable denominator.

Then apply close-year operating contribution once using:

```text
close-year operating fraction = (13 - close month) / 12
```

Validate close month as an integer from 1 to 12 and close year as one of the three forecast years. Treat the acquisition as outside the model horizon otherwise.

Use full-year contribution after the close year. Keep the target EBITDA control and other full-year target metrics unprorated; prorate only the reported adjustment rows.

Add the full acquisition debt balance in the close year. Do not prorate debt or net debt. Calculate close-year acquisition interest using the same inclusive fraction:

```text
close-year interest fraction = (13 - close month) / 12
```

Apply full-year interest after the close year. Flow acquisition EBIT and
interest through pre-tax income, tax and net income before calculating cash
flow, so the selected tax assumption captures the associated tax effect.

Keep acquisition debt outstanding after closing for the remainder of the
...
not create consideration, financing-proceeds, equity-funding or repayment cash
rows. The target's operating contribution and incremental interest still flow
through the ordinary income-statement and operating-cash-flow bridge and hence
affect the combined pro-forma cash/RCF solve; those ordinary consequences are
not direct transaction funding cash flows.

This is a CRH-style reported-basis transaction overlay. Close-year operating contribution and interest are prorated; it is not a conventional full-year close-year transaction pro forma. Produce a separate full-year transaction view only when explicitly requested.

When off:

- every acquisition calculation row that feeds pro forma returns a formula-driven zero;
- structurally non-applicable stub or reference cells may remain blank and grey;
- standalone outputs remain unchanged.

Never represent an ordinary off-state zero with a hardcoded dash or grey fill.

When on:

- adjustment columns show incremental effects;
- pro forma equals standalone plus adjustment;
- debt, interest, cash, liquidity and leverage update coherently.

For linear operating, P&L, cash-flow and debt rows, calculate pro forma as standalone plus adjustment. For non-linear cash, minimum-cash, RCF, capacity and liquidity rows, solve the combined pro-forma waterfall first and calculate adjustment as pro forma minus standalone. Do not push a separately calculated adjustment through `MIN` or `MAX` and assume additivity.

The pro-forma reference column `R` equals the latest standalone historical column `I` and carries no acquisition adjustment. Forecast pro-forma columns `S:U` correspond to standalone `J:L` plus adjustment `N:P`.

The acquisition assumptions and debt schedule remain intact when circularity is
off, but acquisition interest returns formula-driven zero with every other
model-generated forecast interest output.

Validate acquisition restoration with `0 → 1 → 0 → 1`. Standalone cells must remain identical in every state, both on-state results must match after full recalculation and save/reopen, and the off-state must preserve the approved zero/blank roles. Repeat across multiple close years and months.
