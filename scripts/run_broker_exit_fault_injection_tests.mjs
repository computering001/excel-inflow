#!/usr/bin/env node

import assert from "node:assert/strict";
import fs from "node:fs";

import { compileEmergencyZeroBrokerPreview } from "./lib/broker_preview.mjs";
import { DELIVERY_CONSTITUTION } from "./lib/delivery_constitution.mjs";
import { validateJsonSchema } from "./lib/json_schema.mjs";
import {
  executeOptionalBrokerCircuitBreaker,
  optionalBrokerFailureReason,
  validateBrokerBreakerReceipt,
} from "./lib/optional_broker_circuit_breaker.mjs";
import { hashValue } from "./lib/run_store.mjs";
import { superviseBrokerOutcome } from "./lib/broker_supervisor.mjs";

const passState = {
  pipeline_status: "PASS",
  lane_states: {
    filings: { pipeline_status: "PASS", blocker_class: null },
    broker: {
      pipeline_status: "PASS_DEGRADED",
      blocker_class: null,
      summary: { fault_contained_to_zero_authority: true },
    },
    dcs: { pipeline_status: "PASS", blocker_class: null },
  },
};
const internalState = {
  pipeline_status: "BLOCKED_INTERNAL",
  lane_states: {
    filings: { pipeline_status: "PASS", blocker_class: null },
    broker: {
      pipeline_status: "BLOCKED_INTERNAL",
      blocker_class: "INTERNAL_WORK",
      summary: { terminal_reason: "degraded_close_missing_quarantine_receipt" },
    },
    dcs: { pipeline_status: "PASS", blocker_class: null },
  },
  summary: { terminal_reason: "degraded_close_missing_quarantine_receipt" },
};
const postCloseIngressState = {
  pipeline_status: "BLOCKED_INTERNAL",
  blocker_class: "INTERNAL_WORK",
  user_blocking: false,
  lane_states: {
    filings: { pipeline_status: "PASS", blocker_class: null },
    broker: { pipeline_status: "PASS", blocker_class: null },
    dcs: { pipeline_status: "PASS", blocker_class: null },
  },
  summary: { terminal_reason: "attachment_ingress_failed" },
};

const injected = [
  { id: "exception", error: new Error("adapter exploded"), expected: "broker_controller_exception" },
  { id: "timeout", error: new Error("broker adapter timed out"), expected: "broker_timeout" },
  { id: "missing_state", state: null, expected: "broker_invalid_state" },
  { id: "failed_optional_close", state: internalState, expected: "broker_optional_close_failure" },
  { id: "post_close_ingress_failure", state: postCloseIngressState, expected: "broker_optional_close_failure" },
];
let checks = 0;
const previewSchema = JSON.parse(
  fs.readFileSync(new URL("../assets/broker-preview-v1.schema.json", import.meta.url), "utf8"),
);
const breakerReceiptSchema = JSON.parse(
  fs.readFileSync(
    new URL("../assets/optional-broker-breaker-receipt-v1.schema.json", import.meta.url),
    "utf8",
  ),
);
const emergencyPreview = compileEmergencyZeroBrokerPreview({
  bindingHashes: {
    broker_pack_sha256: "a".repeat(64),
    broker_source_tables_sha256: "b".repeat(64),
    broker_crosswalk_receipt_sha256: "c".repeat(64),
  },
  forecastPeriods: ["2026-12-31", "2027-12-31", "2028-12-31"],
  reasons: ["fault injection"],
});
assert.deepEqual(validateJsonSchema(emergencyPreview, previewSchema), []);
checks += 1;
for (const fault of injected) {
  let state = fault.state ?? null;
  let fallbackRuns = 0;
  const result = await executeOptionalBrokerCircuitBreaker({
    runPrimary: async () => {
      if (fault.error) throw fault.error;
    },
    readState: async () => state,
    runZeroAuthority: async () => {
      fallbackRuns += 1;
      state = passState;
    },
  });
  assert.equal(result.state.pipeline_status, "PASS");
  assert.equal(result.circuit_breaker_used, true);
  assert.equal(result.reason_code, fault.expected);
  assert.equal(fallbackRuns, 1);
  assert.deepEqual(validateBrokerBreakerReceipt(result.breaker_receipt), []);
  assert.deepEqual(validateJsonSchema(result.breaker_receipt, breakerReceiptSchema), []);
  assert.equal(result.breaker_receipt.zero_authority_retry_count, 1);
  if (result.breaker_receipt.trigger_mandatory_lane_bindings.length > 0) {
    assert.deepEqual(
      result.breaker_receipt.trigger_mandatory_lane_bindings,
      result.breaker_receipt.mandatory_lane_bindings,
    );
  }
  checks += 8;
}

const mutationState = structuredClone(postCloseIngressState);
let mutationFinalState = mutationState;
const mutationControl = await executeOptionalBrokerCircuitBreaker({
  runPrimary: async () => ({ code: 2, stderr: "post-close ingress failure" }),
  readState: async () => mutationFinalState,
  runZeroAuthority: async () => {
    mutationFinalState = structuredClone(passState);
    return { code: 0 };
  },
});
const selfHashMutation = structuredClone(mutationControl.breaker_receipt);
selfHashMutation.failure_count += 1;
assert.ok(validateBrokerBreakerReceipt(selfHashMutation).includes(
  "receipt_sha256 does not bind the breaker receipt body",
));
const retryCountMutation = structuredClone(mutationControl.breaker_receipt);
retryCountMutation.zero_authority_retry_count = 0;
retryCountMutation.receipt_sha256 = hashValue((({ receipt_sha256, ...body }) => body)(retryCountMutation));
assert.ok(validateBrokerBreakerReceipt(retryCountMutation).includes(
  "zero_authority execution and retry count disagree",
));
const mandatoryHashMutation = structuredClone(mutationControl.breaker_receipt);
mandatoryHashMutation.mandatory_lane_bindings[0].state_sha256 = "f".repeat(64);
mandatoryHashMutation.receipt_sha256 = hashValue((({ receipt_sha256, ...body }) => body)(mandatoryHashMutation));
assert.ok(validateBrokerBreakerReceipt(mandatoryHashMutation).includes(
  "a zero-authority retry changed mandatory lane state",
));
checks += 3;

let staleFallbackState = structuredClone(postCloseIngressState);
await assert.rejects(
  executeOptionalBrokerCircuitBreaker({
    runPrimary: async () => ({ code: 2, stderr: "post-close ingress failure" }),
    readState: async () => staleFallbackState,
    runZeroAuthority: async () => ({ code: 0 }),
  }),
  /cannot infer zero authority from a successful process exit alone/,
  "a zero-authority process exit without a fresh zero-authority state was accepted",
);
checks += 1;

let mandatoryFallbackRuns = 0;
const mandatoryOpenState = {
  pipeline_status: "BLOCKED_INTERNAL",
  blocker_class: "INTERNAL_WORK",
  user_blocking: false,
  lane_states: {
    filings: {
      pipeline_status: "BLOCKED_INTERNAL",
      blocker_class: "INTERNAL_WORK",
    },
    broker: { pipeline_status: "PASS", blocker_class: null },
    dcs: { pipeline_status: "PASS", blocker_class: null },
  },
};
const mandatoryResult = await executeOptionalBrokerCircuitBreaker({
  runPrimary: async () => ({ code: 2, stderr: "mandatory filings failure" }),
  readState: async () => mandatoryOpenState,
  runZeroAuthority: async () => {
    mandatoryFallbackRuns += 1;
  },
});
assert.equal(mandatoryResult.circuit_breaker_used, false);
assert.equal(mandatoryFallbackRuns, 0);
assert.equal(
  mandatoryResult.state.lane_states.filings.pipeline_status,
  "BLOCKED_INTERNAL",
);
checks += 3;

assert.equal(optionalBrokerFailureReason({ state: {
  summary: { terminal_reason: "invalid_lane_state" },
} }), "broker_invalid_state");
checks += 1;

for (const reasonCode of DELIVERY_CONSTITUTION.degrade_reasons.filter(
  (reason) => reason.startsWith("broker_"),
)) {
  const supervised = superviseBrokerOutcome({
    outcome: "failed",
    reasonCode,
    attemptedSelection: !["broker_zero_usable_houses"].includes(reasonCode),
  });
  assert.equal(supervised.terminal_state, "CLOSED");
  assert.equal(supervised.authority_mode, "zero_broker_authority");
  assert.equal(supervised.selected_cell_count, 0);
  checks += 3;
}

console.log(JSON.stringify({
  schema_version: "broker-exit-fault-injection-report/1.0",
  status: "PASS",
  injected_exit_classes: injected.length,
  constitution_broker_reasons: DELIVERY_CONSTITUTION.degrade_reasons.filter(
    (reason) => reason.startsWith("broker_"),
  ).length,
  checks,
  broker_caused_hard_blocks: 0,
}, null, 2));
