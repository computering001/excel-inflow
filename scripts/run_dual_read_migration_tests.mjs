    ...
    for (const row of modelCase.statement_structure?.[section] ?? []) {
      ...
      for (let periodIndex = 0; periodIndex < 6; periodIndex += 1) {
        ...
        if ((raw === null || raw === undefined) && view === 0) {
          check(false, `${row.row_id}[${periodIndex}]: an absent legacy value read as ZERO`);
        }
...
check(projected > 500, `corpus projection must cover a substantial surface (got ${projected})`);
check(valueBearing > 100, `corpus must carry real values (got ${valueBearing})`);
check((stateCounts.missing ?? 0) > 0, "the corpus exercises the missing state");
check((stateCounts.reported_zero ?? 0) > 0 || (stateCounts.derived_number ?? 0) > 0,
...
// must be refused by the contract — the dual read cannot launder it.
{
  ...
  check(validate(smuggled).length > 0, "a smuggled zero on missing must be refused");
}
...
// 5. Vocabulary compatibility: extractor value_states map losslessly.
{
  ...
  check(stitched.state === "prior_filing_support" && stitched.value === 12.5 &&
  ...
  check(owed.state === "period_support_required" && legacyViewOf(owed) === null,
    "an owed period projects typed and reads null, never zero");
}

console.log(JSON.stringify({
  status: "PASS", checks, projected, value_bearing: valueBearing,
