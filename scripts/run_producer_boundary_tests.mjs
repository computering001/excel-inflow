...
const run = createRunner({ name: "producer_boundary_tests", importMetaUrl: import.meta.url });

// 1. anyOf enforcement: value must match at least one branch.
const anySchema = { anyOf: [{ type: "string" }, { type: "null" }] };
run.ok(validateJsonSchema("x", anySchema).length === 0, "anyOf accepts a matching branch");
run.ok(validateJsonSchema(null, anySchema).length === 0, "anyOf accepts the null branch");
run.ok(validateJsonSchema(7, anySchema).length === 1, "anyOf REFUSES a value matching no branch");
...
run.ok(validateJsonSchema({ a: 1 }, oneSchema).length === 0, "oneOf accepts exactly one match");
run.ok(validateJsonSchema({ c: 1 }, oneSchema).length > 0, "oneOf refuses zero matches");
const ambiguous = { oneOf: [{ type: "number" }, { type: "number", minimum: 0 }] };
run.ok(
  validateJsonSchema(5, ambiguous).some((error) => error.includes("matched 2")),
...
const modelSchema = JSON.parse(
  await fs.readFile(path.join(run.ROOT, "assets", "model-case-v2.schema.json"), "utf8"),
);
const probe = {
  ...modelSchema,
...
run.ok(
  ...
  "the opening-cash temporal_edge rule is a declared lawful shape",
);
run.ok(
  validateJsonSchema(
    { probe: { operator: "prior_period", refs: ["ending_cash"], smuggled: true } }, probe,
  ).length > 0,
  "an undeclared formulaRule field still refuses",
);
...
for (const entry of await fs.readdir(path.join(run.ROOT, "assets"))) {
  if (!entry.endsWith(".schema.json")) continue;
  const body = await fs.readFile(path.join(run.ROOT, "assets", entry), "utf8");
  constructCount += (body.match(/"oneOf"|"anyOf"/g) ?? []).length;
}
run.ok(constructCount > 10, `shipped schemas declare ${constructCount} oneOf/anyOf constructs — all now enforced`);

run.finish({ enforced_constructs: constructCount });
